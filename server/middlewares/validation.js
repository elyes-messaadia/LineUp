...
const validationRules = {
  ...
  password: {
    ...
    message: 'Mot de passe trop faible (8+ caractères, maj, min, chiffre, symbole)'
  },
  
  firstName: {
    required: true,
    validator: (value) => validator.isLength(value, { min: 2, max: 50 }) && validator.isAlpha(value, 'fr-FR', { ignore: ' \'-' }),
  ...
  role: {
    required: true,
    validator: (value) => ['patient', 'docteur', 'secretaire', 'admin'].includes(value),
...
const validateField = (field, value, rules = validationRules) => {
  ...
  // Sanitisation
  const sanitizedValue = rule.sanitizer ? rule.sanitizer(value) : sanitize.text(value);
...
const createRegistrationRateLimit = () => rateLimit({
