...
export default function SecretaryDashboard() {
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      ...
      case 'normal': return 'text-secondary-600 bg-secondary-100';
      default: return 'text-secondary-600 bg-secondary-100';
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-accessible border border-secondary-200 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-accent-600">{stats.waitingListCount}</p>
              <p className="text-xs text-secondary-600">Liste d'attente</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-error-600">{stats.urgentWaiting}</p>
      ...
      {selectedView === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-accessible border border-secondary-200 p-6">
            ...
            <div className="space-y-3">
              {appointments.filter(apt => apt.status === 'pending').slice(0, 3).map((appointment) => (
                <div key={appointment.id} className="p-3 bg-warning-50 rounded-lg border border-warning-200">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-medium text-secondary-800">{appointment.patient}</p>
                    <span className="text-xs text-warning-600">{appointment.time}</span>
                  </div>
                  <p className="text-sm text-secondary-600 mb-2">{appointment.reason}</p>
          ...
          <div className="bg-white rounded-xl shadow-accessible border border-secondary-200 p-6">
            ...
            <div className="space-y-3">
              {waitingList.filter(w => w.priority === 'urgent').map((waiting) => (
                <div key={waiting.id} className="p-3 bg-error-50 rounded-lg border border-error-200">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-medium text-secondary-800">{waiting.patient}</p>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(waiting.priority)}`}>
                      URGENT
                    </span>
                  </div>
            ...
            <div className="p-6 border-b border-secondary-200">
              <h2 className="text-xl font-semibold text-secondary-800 flex items-center gap-2">
                ...
                <span className="text-sm font-normal text-secondary-600">
                  ({filteredAppointments.filter(apt => apt.date === selectedDate).length})
                </span>
              </h2>
            </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    ...
                    <div className="lg:col-span-2">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <h3 className="font-medium text-secondary-800">{appointment.patient}</h3>
                          <p className="text-sm text-secondary-600">{appointment.phone}</p>
                        </div>
                      </div>
                    ...
                    <div>
                      <p className="text-sm text-secondary-600 mb-1">Médecin :</p>
                      <p className="font-medium text-secondary-800">{appointment.doctor}</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="lg:col-span-2">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <h3 className="font-medium text-secondary-800">{patient.name}</h3>
                  ...
                  <div className="text-right">
                    <p className="text-sm text-secondary-600 mb-1">Dernière visite :</p>
                    <p className="font-medium text-secondary-800">{formatDate(patient.lastVisit)}</p>
                    <p className="text-sm text-secondary-600">{patient.totalVisits} consultations</p>
        ...
        <div className="bg-white rounded-xl shadow-accessible border border-secondary-200">
          ...
          <div className="divide-y divide-secondary-200">
            {waitingList.map((waiting) => (
              <div key={waiting.id} className="p-6 hover:bg-secondary-50 transition-colors duration-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-warning-100 rounded-full flex items-center justify-center">
                      <Icon name="clock" size="sm" className="text-warning-600" />
                    </div>
                    <div>
                      <h3 className="font-medium text-secondary-800">{waiting.patient}</h3>
                      <p className="text-sm text-secondary-600">{waiting.phone}</p>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(waiting.priority)}`}>
