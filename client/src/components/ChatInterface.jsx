...
import { Send, MessageCircle, AlertCircle, Clock, User, Bot } from 'lucide-react';
    ...
    <div className="flex flex-col h-[600px] bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-blue-50">
        ...
        {conversation?.urgencyLevel && (
          <div className={`px-3 py-1 rounded-full text-sm font-medium ${getUrgencyColor(conversation.urgencyLevel)}`}>
      ...
      <div className="border-t border-gray-200 p-4">
        <div className="flex items-end space-x-2">
          ...
          <button
            onClick={sendMessage}
            disabled={!newMessage.trim() || isLoading || conversation?.status === 'termine'}
        ...
        {conversation?.status === 'termine' && (
          <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-sm text-green-700">
            <div className="flex items-center space-x-1">
              ...
              <span>Évaluation terminée. Un membre de notre équipe va prendre contact avec vous.</span>
            </div>
          </div>
        )}
      </div>
