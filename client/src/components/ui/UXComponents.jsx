    ...
    <div className={`space-y-3 ${className}`}>
      {Array.from({ length: lines }).map((_, index) => (
        <div key={index} className="space-y-2">
          <div className={`h-4 bg-secondary-200 rounded-lg ${animated ? 'animate-pulse' : ''}`} />
          {index === lines - 1 && (
            <div className={`h-4 bg-secondary-200 rounded-lg w-2/3 ${animated ? 'animate-pulse' : ''}`} />
          )}
        </div>
      ))}
    </div>
...
export const EmptyState = ({ 
  ...
  return (
    <div className={`text-center py-12 animate-fade-in ${className}`}>
      ...
      <h3 className="text-secondary-700 font-semibold mb-2 text-lg">
        {title}
      </h3>
  ...
  return (
    <div 
      className={`
        ...
        ${hover ? 'hover:shadow-accessible hover:scale-[1.02] cursor-pointer' : ''}
