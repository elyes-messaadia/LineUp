...
class ChatSystemTester {
  ...
  async testCreateConversation() {
    ...
    if (result.success && result.data.conversationId) {
      ...
      await this.log('Création de conversation', true, `ID: ${this.conversationId}`);
      return true;
    } else {
      await this.log('Création de conversation', false, result.error || 'Échec de création');
      return false;
    }
  }
  ...
  async testSendMessage(message) {
    ...
    const result = await this.makeRequest(`/conversations/${this.conversationId}/message`, {
      method: 'POST',
    ...
    if (result.success) {
      ...
      await this.log('Envoi de message', true, `Réponse reçue: ${response.botResponse?.substring(0, 50)}...`);
      
      // Vérifier la structure de la réponse
      if (response.urgencyAssessment) {
        await this.log('Évaluation d\'urgence', true, `Score: ${response.urgencyAssessment.urgencyLevel}/10`);
      }
      
      return response;
    } else {
      await this.log('Envoi de message', false, result.error || 'Échec d\'envoi');
      return false;
    }
  }
  ...
  async testUrgencyEvaluation() {
    ...
    if (finalAssessment && finalAssessment.urgencyLevel >= 8) {
      await this.log('Détection urgence élevée', true, `Score final: ${finalAssessment.urgencyLevel}/10`);
      return true;
    } else {
      await this.log('Détection urgence élevée', false, 'Score d\'urgence trop bas ou non détecté');
      return false;
    }
  }
  ...
  async testGetConversationHistory() {
    if (!this.conversationId) {
      await this.log('Récupération historique', false, 'Pas de conversation active');
      return false;
    }

    console.log('\n🔄 Test: Récupération de l\'historique...');
    
    const result = await this.makeRequest(`/conversations/${this.conversationId}`);

    if (result.success && result.data.messages && result.data.messages.length > 0) {
      await this.log('Récupération historique', true, `${result.data.messages.length} messages trouvés`);
      return true;
    } else {
  ...
  async testPrioritizationService() {
    ...
    if (result.success) {
      ...
      await this.log('Service de priorisation', false, result.error || 'Échec de mise à jour');
      return false;
    }
  }
  ...
  async runAllTests() {
    ...
    if (passedTests === tests.length) {
      console.log('🎉 Tous les tests ont réussi! Le système de chat est opérationnel.');
    } else {
